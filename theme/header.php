<html>
<head>
	<title>Plusify Blog</title>
	<link rel="stylesheet" type="text/css" media="screen" href="<?=$this->SETTINGS_ROOT_URL?>?style=true" />
	<script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.6.4/jquery.min.js"></script>
	<script type="text/javascript" src="http://ajax.microsoft.com/ajax/jquery.cycle/2.88/jquery.cycle.all.js"></script>
</head>
<body>
	<?php include_once($this->SETTINGS_TEMPLATE_DIR . 'sidebar.php');?>
	<div id="main">
	<div id="header">
		<h1><a href="<?=$this->SETTINGS_ROOT_URL?>">{{#person}}{{display_name}}'s{{/person}} Blog</a></h1>
	</div>
	<div id="content">
